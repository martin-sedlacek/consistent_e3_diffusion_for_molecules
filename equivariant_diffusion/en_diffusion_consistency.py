...
class ConsistentEnVariationalDiffusion(torch.nn.Module):
    """
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, t0_always):
        ...
        # TODO: rest of forward pass from consistency models - might have issues
        orig_eps = 0.002
        t = t - orig_eps
        c_skip_t = 0.25 / (t.pow(2) + 0.25)
        c_out_t = 0.25 * t / ((t + orig_eps).pow(2) + 0.25).pow(0.5)
        net_out = c_skip_t[:, :, None] * xh + c_out_t[:, :, None] * net_out
    ...
    def make_pred(self, x, h, t_int, node_mask, edge_mask, context):
        t = t_int / self.T
        gamma_t = self.inflate_batch_array(self.gamma(t), x)
